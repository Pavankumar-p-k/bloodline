      ...
      <nav className="w-full bg-surface border-b border-border sticky top-0 z-40 text-text h-16 flex items-center shadow-md">
        <div className="container mx-auto px-4 flex items-center justify-between relative">
          <Link href="/" className="font-black text-xl flex items-center gap-1.5 text-white tracking-wider hover:opacity-90">
            <span className="text-vital text-2xl">ðŸ©¸</span>
            <span>BLOOD<span className="text-vital">LINE</span></span>
          </Link>
          
          <div className="hidden sm:flex items-center gap-6 text-sm font-semibold">
            <Link href="/" className={`transition-all hover:text-vital ${pathname === '/' ? 'text-vital' : 'text-text'}`}>
              Home
            </Link>
            <Link href="/map" className={`transition-all hover:text-vital ${pathname?.startsWith('/map') ? 'text-vital' : 'text-text'}`}>
              Live Map
            </Link>
            <Link href="/emergency" className={`transition-all hover:text-vital ${pathname?.startsWith('/emergency') ? 'text-vital' : 'text-text'}`}>
              Emergency Request
            </Link>
            ...
            {user ? (
              <>
                ...
                {role === "donor" && (
                  <div className="relative">
                    <button 
                      ...
                      className="p-1.5 rounded-lg hover:bg-surface-2 text-text hover:text-white transition-all relative"
                    >
                      <Bell className="h-5 w-5" />
                      {notifications.length > 0 && (
                        <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-vital border-2 border-[#1A1A1A] rounded-full flex items-center justify-center text-[8px] text-white font-black animate-bounce">
                      ...
                      <div className="absolute right-0 mt-2.5 w-72 bg-surface border border-border rounded-xl shadow-2xl p-4 z-50 text-left space-y-3">
                        <div className="flex items-center justify-between border-b border-border pb-2">
                          <span className="text-xs font-extrabold uppercase text-white tracking-wider flex items-center gap-1">
                            <Bell className="h-3.5 w-3.5 text-vital" /> Alerts ({notifications.length})
                          </span>
                          {notifications.length > 0 && (
                            <button 
                              onClick={markAllAsRead}
                              className="text-[10px] text-text-2 hover:text-white flex items-center gap-0.5"
                            >
                          ...
                          {notifications.length === 0 ? (
                            ...
                            notifications.map(n => (
                              ...
                              >
                                ...
                                <span className="text-[9px] text-text-2 block">
                ...
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-lg bg-vital-dim border border-[#C41E3A]/40 text-vital hover:bg-vital hover:text-white transition-all text-xs font-bold"
                >
                  Sign Out
                </button>
              </>
              ...
              <div className="flex items-center gap-3">
                <Link
                  href="/auth/login"
                  className="px-3 py-1.5 text-text hover:text-white transition-all text-xs font-bold"
                >
                  Login
                </Link>
                <Link
                  href="/auth/signup"
                  className="px-3.5 py-1.5 rounded-lg bg-vital hover:bg-vital text-white transition-all text-xs font-bold shadow-[0_0_8px_rgba(196,30,58,0.2)]"
                >
                  Join Us
                </Link>
              </div>
          ...
          <div className="sm:hidden flex items-center gap-3">
            {user && role === "donor" && (
              <button 
                ...
                className="relative text-text-2 hover:text-white"
              >
                <Bell className="h-5 w-5" />
                {notifications.length > 0 && (
                  <span className="absolute -top-1 -right-1 w-3 h-3 bg-vital rounded-full flex items-center justify-center text-[7px] text-white font-bold">
                    {notifications.length}
                  </span>
                )}
              </button>
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 h-16 bg-surface border-t border-border z-40 flex items-center justify-around text-text-2">
        ...
        <Link 
          ...
          className={`flex flex-col items-center justify-center w-16 h-full transition-all ${
            isTabActive("/") ? "text-vital" : "hover:text-text"
          }`}
        >
        ...
        <Link 
          href="/map" 
          className={`flex flex-col items-center justify-center w-16 h-full transition-all ${
            isTabActive("/map") ? "text-vital" : "hover:text-text"
          }`}
        >
        ...
        <Link 
          href="/emergency" 
          className={`flex flex-col items-center justify-center w-16 h-full transition-all ${
            isTabActive("/emergency") ? "text-vital" : "hover:text-text"
          }`}
        >
        ...
        <Link 
          href={getProfilePath()} 
          className={`flex flex-col items-center justify-center w-16 h-full transition-all ${
            pathname?.includes("dashboard") || pathname?.startsWith("/auth") ? "text-vital" : "hover:text-text"
          }`}
        >
