...
export default function EmergencyRequestPage() {
  ...
  return (
    <div className="bg-void min-h-screen pb-16">
      {/* Top Red Urgency Banner */}
      <div className="bg-vital text-white py-3.5 px-4 shadow-[0_4px_20px_rgba(196,30,58,0.3)] animate-pulse">
        <div className="max-w-4xl mx-auto flex items-center justify-center gap-2">
          ...
          <span className="font-extrabold text-sm uppercase tracking-widest text-center">
            EMERGENCY REQUEST â€” We will contact donors immediately
          </span>
        </div>
      </div>

      <div className="max-w-2xl mx-auto px-4 mt-8">
        <form onSubmit={handleSubmitRequest} className="bg-surface border border-border rounded-2xl p-6 sm:p-8 space-y-6 shadow-2xl">
          
          <div className="flex items-center gap-3 border-b border-border pb-4">
            <ShieldAlert className="h-6 w-6 text-vital" />
            <div>
              <h2 className="text-xl font-bold text-text">Emergency Broadcast Form</h2>
              <p className="text-xs text-text-2">Submit details to initiate immediate donor response matching</p>
            </div>
          </div>
            ...
            {['Patient Family', 'Hospital', 'Clinic', 'Blood Bank'].map(type => (
              <button
                ...
                className={`py-2 px-3 rounded-lg border text-xs font-bold transition-all text-center ${
                  requesterType === type 
                    ? 'border-vital bg-vital-dim text-vital' 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-text-2 uppercase mb-1">Patient Name (Optional)</label>
              <input 
                type="text" 
            ...
            <div>
              <label className="block text-xs font-semibold text-text-2 uppercase mb-1">Urgency Level</label>
              <select
                value={urgencyLevel}
          ...
          <div>
            <label className="block text-xs font-semibold text-text-2 uppercase mb-3">Blood Type Needed</label>
              ...
              {['A+', 'A-', 'B+', 'B-', 'O+', 'O-', 'AB+', 'AB-'].map(bg => (
                <button
                  ...
                  className={`p-3 rounded-xl border flex flex-col items-center justify-center transition-all ${
                    bloodGroup === bg 
                      ? 'border-vital bg-vital-dim text-vital shadow-vital-glow' 
                      : 'border-border bg-surface-2 text-text-2 hover:border-border-2'
                  }`}
                >
          ...
          <div>
            <div className="flex justify-between items-center text-xs font-semibold text-text-2 uppercase mb-1">
              <span>Units Needed</span>
              <span className="text-vital font-bold text-sm">{units} Bags ({units * 350} ml)</span>
            </div>
            <input 
              type="range" 
              min={1} 
              ...
              className="w-full accent-vital bg-surface-2 h-1.5 rounded-lg"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-text-2 uppercase mb-1">Hospital / Clinic Name</label>
              <input 
                type="text" 
                required
            ...
            <div>
              <label className="block text-xs font-semibold text-text-2 uppercase mb-1">Hospital Full Address</label>
              <input 
                type="text" 
                required
          ...
          <div className="bg-surface-2 border border-border rounded-xl p-4 space-y-3">
            <h4 className="text-xs font-semibold text-text-2 uppercase">Verification Document</h4>
            ...
            <div className="flex items-center gap-4">
              <label className="cursor-pointer bg-surface hover:bg-surface-2 border border-border text-text text-xs font-bold py-2 px-3 rounded-lg flex items-center gap-1.5 transition-all">
                <Upload className="h-4 w-4 text-vital" /> 
              ...
              {docUrl && (
                <span className="text-confirmed text-xs font-bold flex items-center gap-1">
          ...
          <div className="border-t border-border pt-6 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-text-2 uppercase mb-1">Contact Person Name</label>
                <input 
                  type="text" 
                  required
              ...
              <div>
                <label className="block text-xs font-semibold text-text-2 uppercase mb-1">Contact Phone (OTP Required)</label>
                <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-1 bg-surface-2 border border-border rounded-xl px-4 py-3 text-sm text-text focus:outline-none focus:ring-2 focus:ring-vital disabled:opacity-50"
                  />
                  {!phoneVerified ? (
                    <button 
                      type="button" 
                      onClick={sendOTP}
                      className="px-3 bg-vital-dim border border-vital/40 text-vital hover:bg-vital hover:text-white rounded-xl text-xs font-bold transition-all flex items-center gap-1"
                    >
                      <Smartphone className="h-3.5 w-3.5" /> Send OTP
                    </button>
                  ) : (
                    <span className="px-3 bg-confirmed/5 text-confirmed border border-confirmed/20 rounded-xl text-xs font-bold flex items-center gap-1">
            ...
            {otpSent && !phoneVerified && (
              <div className="bg-surface-2 p-4 rounded-xl border border-border space-y-2">
                <p className="text-xs text-text-2">Enter 6-digit OTP verification code:</p>
                <div className="flex gap-2">
                  <input 
                    type="text" 
          ...
          {/* Notes */}
          <div>
            <label className="block text-xs font-semibold text-text-2 uppercase mb-1">Additional Notes / Requirements</label>
            <textarea 
              rows={3}
          ...
          {matchedDonorsCount !== null && (
            <div className="bg-vital-dim border border-vital/40 rounded-xl p-4 text-center">
              <span className="text-vital font-extrabold text-sm animate-pulse">
                â¤ï¸ Found {matchedDonorsCount} active, eligible {bloodGroup || "matching"} donors in broadcasting range.
              </span>
            </div>
          )}
          ...
          <div className="pt-4 border-t border-border flex justify-end">
            <button
              ...
              className="w-full sm:w-auto px-8 py-3.5 bg-vital hover:bg-vital text-white font-bold rounded-lg text-sm transition-all disabled:opacity-40 flex items-center justify-center gap-1.5"
            >
