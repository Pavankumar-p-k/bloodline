...
export default function AboutPage() {
  return (
    <main className="min-h-[85vh] bg-void text-text py-12 px-6">
      <div className="mx-auto max-w-4xl space-y-10">
        
        {/* Header */}
        <div className="border-b border-border pb-6">
          <span className="text-xs font-semibold text-vital uppercase tracking-widest block">About the platform</span>
          <h1 className="text-4xl font-black text-white mt-1">Our Mission & Design</h1>
        </div>

        {/* Introduction */}
        <div className="bg-surface border border-border rounded-2xl p-6 sm:p-8 shadow-xl space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-surface border border-border p-5 rounded-xl space-y-2">
            ...
            <p className="text-xs text-text-2">Matches compatible blood groups (including O- universal falls) within the donor's exact travel limits.</p>
          </div>

          <div className="bg-surface border border-border p-5 rounded-xl space-y-2">
            <Clock className="h-6 w-6 text-vital" />
            <h3 className="font-bold text-white text-sm uppercase tracking-wider">Immediate Dispatch</h3>
            <p className="text-xs text-text-2">Dispatches automated notifications and lists matching emergency request pins on a live map within seconds.</p>
          </div>
          ...
          <div className="bg-surface border border-border p-5 rounded-xl space-y-2">
            ...
            <p className="text-xs text-text-2">Hides exact donor coordinates, showing only city-level areas. Contact info is revealed only after donor confirmation.</p>
          </div>

          <div className="bg-surface border border-border p-5 rounded-xl space-y-2">
            <Map className="h-6 w-6 text-vital" />
            <h3 className="font-bold text-white text-sm uppercase tracking-wider">Hospital Command</h3>
            <p className="text-xs text-text-2">Allows coordinators to upload verification slips, audit queues, and monitor active en-route travel ETAs.</p>
          </div>
        </div>

        {/* Call to action */}
        <div className="text-center pt-6 text-xs text-text-3 font-semibold border-t border-border">
          Â© 2026 Bloodline Network. Registered Health Service. National Emergency: +91 11-4567-8910.
        </div>
